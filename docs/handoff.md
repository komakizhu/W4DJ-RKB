...
最近更新：2026-09-06（W4DJ 3.2.4 历史歌单入口与重复导入隔离）
...
### 2026-09-03 W4DJ 歌单导出根因修复

`.w4dj` 不升级格式，继续使用 v2；`netease_track_id` 在 wire 层仅为旧文件兼容字段，新的领域模型、输出身份和导出均不使用它，重新导出固定为 `null`。网易云 ID 只在转换阶段用于本地元数据检索。成功转换输出登记 `conversion_batch_id`，歌单导入后最近批次优先以标题/歌手 BM25F 风格分数一一匹配，历史库只用 50 分及以上候选补缺。

M3U8 导出前现在打开两栏逐行复核：左边是歌单位置和标题/歌手，右边是实际本地文件或空白选择入口。有效自动绑定默认可导出，不再显示或要求“确认此行”；未绑定行可手动选择本地音频。左栏复选框只用于从当前导出列表移除歌曲，支持单首删除、批量删除、全选和恢复；删除只保存歌单复核状态，不删除 W4DJ 曲库记录或本地音频。后端只导出未移除且路径仍可读的行，避免把用户明确移除的歌曲带入 M3U8。隐藏恢复清单只在显式 W4DJ 操作写入，普通转换不保存。

主样本 `/Users/mac2/Library/Messages/Attachments/d6/06/F9E72671-D45C-4DCA-94FE-EE5AE9A0AD6E/john-summit-edm-house-set.w4dj` 已用 8 个短静音源完成真实普通转换验收；冻结补充数据集还覆盖了格式归一化、12 对 10 冗余候选、4+2 历史补齐和低置信度手动恢复，均生成完整 M3U8 且未省略条目。验收数据位于 `/Users/mac2/Documents/W4DJ RKB/test-artifacts/w4dj/`，为 Git 忽略的本机测试夹具。
