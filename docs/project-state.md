...
## 2026-09-03 W4DJ v2 歌单复核与完整 M3U8

当前开发分支为 `3.2.3`，产品版本定义已同步为 `3.2.3`。`.w4dj` 继续保持 `format_version: 2`；歌单轨道的 `netease_track_id` 只在 wire 层兼容读取，领域模型不携带它，重新导出固定写 JSON `null`。网易云 ID 仅存在于转换阶段的本地元数据检索边界，不参与歌单导入、输出身份、匹配、隐藏恢复清单、复核 UI、运行报告或 M3U8。
