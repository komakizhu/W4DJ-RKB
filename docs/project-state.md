...
## 2026-09-06 W4DJ v3.2.4 历史歌单入口与重复导入隔离

当前开发分支为 `3.2.4`，产品版本定义已同步为 `3.2.4`。`.w4dj` 继续保持 `format_version: 2`；歌单轨道的 `netease_track_id` 只在 wire 层兼容读取，领域模型不携带它，重新导出固定写 JSON `null`。网易云 ID 仅存在于转换阶段的本地元数据检索边界，不参与歌单导入、输出身份、匹配、隐藏恢复清单、复核 UI、运行报告或 M3U8。

W4DJ 输出库现在记录成功安全提交的 `conversion_batch_id` 和提交时间。歌单导入后的匹配优先消费最近成功批次，使用标题 65% / 歌手 35% 的 BM25F 风格绝对分数和字符 trigram 归一化；最近批次按一一分配处理，缺口才从历史库使用 `score >= 50` 的候选补齐。自动匹配不再静默丢歌，匹配不足的行保留为空供手动选本地音频。

导出流程已改为复用扫描后转换的 modal 外壳，主体固定为歌单歌曲/本地歌曲两栏，逐行横向对应。有效自动匹配默认可导出，不再要求用户逐行确认；未绑定行保留“选择本地歌曲”入口。复选框只用于选择要从当前导出列表移除的歌曲，支持单首删除、批量删除、全选和恢复；删除只保存歌单的导出复核状态，不删除 W4DJ 曲库记录或本地音频。导出时只写入未移除且有当前可读路径的行，显式 W4DJ 操作才写入不含网易云 ID 的隐藏恢复清单，普通转换不写。

验收夹具已冻结在 `test-artifacts/w4dj/`。用户提供的 John Summit 8 首歌单经过短静音源、真实普通 FFmpeg 转换、批次登记、BM25F 匹配、两栏复核和 M3U8 生成，结果为 8/8；补充矩阵覆盖 8 首归一化、10 首歌单/12 个最近候选、4 最近+2 历史补齐和低置信度手动恢复，均完成且 `omitted=0`。这些夹具只验证管线，不代表网易云下载内容本身正确。
...
已把网易云轻量 locator 的持久化键与运行时宽松比较键拆开：持久化键只做首尾 trim/lowercase，内部空格、逗号空格、全半角标点和弯引号不再被压平；旧 cache schema `1` 判 stale，当前 schema 为 `2`。locator 与正式网易云身份分离，预览不得把 locator 键当正式标签；匹配后由只读 resolver 重新读取原始数据库行。

元数据现在逐字段按“原文件有效值 → 网易云正式数据库 → 可靠文件名”选择，诊断记录字段来源及 exact/caseOnly/whitespaceOnly/punctuationOnly/different 差异。Title、Artist、Album 都参与最终输出复读校验；W4DJ 登记改用最终容器实际复读文本，缺失标签不再用候选键静默兜底。已新增持久化键、旧 schema、专辑补全、正式输出复读和缺失标签拒绝回归。

544 条真实 locator 只读审计为 544/544 行可回读；生产缓存当前仍为旧 `schemaVersion=1`，新代码会安全判 stale 并重建。按新持久化键规则重算发现 224 个字段级格式差异、涉及 207 个 Track ID；审计未写入用户数据库、音频或缓存。最终验证中根库 `cargo test --all`、Tauri 74/74、前端 Vitest 12 文件/209 项、TypeScript、Vite、cargo check、fmt、diff-check、Tauri 严格 Clippy 以及根库 `-A dead_code` all-targets Clippy 均通过。根库不抑制 dead-code 的严格 all-targets 仍保留历史未使用 API 诊断，未伪报通过。最新版 arm64 App 已在最终代码上重编译，版本仍为 `3.2.0-beta.3`，未修改用户数据或 Git 远端。
...
## 2026-08-29 beta-3 定稿边界

beta-3 已在本地 `main` 定稿为 `ae8c116`，未 push。已跟踪代码干净；Git 仍会显示 `.pnpm-store/`、`app/public/acceptance-audio`、`app/public/essentia-models`、`docs/.DS_Store` 和 `src-tauri/.DS_Store` 五项未跟踪本机资源。其中两个 `app/public` 项是工作区验收软链接，不进入 `app/dist` 或正式 App，不得提交绝对路径或误删链接目标。完整资源说明和下一版待验收清单见 `docs/handoff.md` 同日章节。

延后到下一版的环境依赖验收包括 Windows/Windows Hidden、Rekordbox/CDJ、macOS Hidden 真实标记、真实大库现场扫描与转换、扫描→转换→增强分析→回写整链路，以及人工 GUI/播放器确认。这些项目当前仍是“未执行/受环境限制”，不能因 beta-3 定稿而改成已验收。

## 2026-08-29 任务卡扫描/转换状态栏

`scan_then_convert` 现在会在确认预览后结束 completed 扫描快照的显示生命周期，不再用“扫描成功 x/x”遮挡转换数量。任务运行/完成/失败/取消分别显示“正在转换/转换完成/转换失败/转换已取消 completed/total”。任务 footer 使用固定状态行 + 独立进度条，左右文案为 `9px`、`line-height: 1.35`并基线对齐；任务 2 保留相同行高。切换主题、打开历史或歌曲库不清除扫描成功结果，来源变化只清除对应任务快照。Rust 转换、历史和数据库接口未修改。

前端全量 12 文件/209 项、TypeScript、Vite 和 diff-check 通过。最新 arm64 App 构建于 2026-08-29 23:34:19 CST，版本保持 `3.2.0-beta.3`，约 96 MB。

## 2026-08-30 跨格式覆盖状态

跨格式覆盖现以稳定歌曲身份而不是目标扩展名判断。预览保留同一身份在当前输出根内的全部有效副本；WAV→AIFF 等切换会显示逻辑歌曲覆盖数，MP3 来源在无损模式下仍保持 MP3。可靠标签冲突或身份不足的同名输出不会进入删除集合。

转换成功后的最终化顺序为：新文件安全提交、ffprobe/关键标签复读、W4DJ 库登记、逐个清理已确认旧路径。歌曲库不可用、复读失败或旧路径删除失败会把该曲记为“覆盖失败”并从完成数撤回；报告保留新旧路径和原因。Skip 已支持跨格式命中，仅更新元数据会更新全部确认副本且保持容器不变。Rename 冲突策略已从 Rust、TypeScript、预览和历史标签中删除。

临时目录真实验收使用 FFmpeg 生成 4 首合法 FLAC 和 2 首合法 MP3：首轮 WAV 输出为 4 WAV+2 MP3，次轮 AIFF 覆盖的预览为新增 0、已有/覆盖 6，完成 6/6 后目录恰为 4 AIFF+2 MP3、旧 WAV 为 0；测试目录已清理，未触碰用户音乐。Rust 全量、Tauri 定向、前端 209/209、TypeScript、Vite 均通过。

最新版 `3.2.0-beta.3` arm64 App 于 2026-08-30 01:30 CST 构建，大小约 93 MiB。工作区验收 CLI 现需显式 `acceptance-tools` feature，未进入 App；App 内 `Contents/MacOS` 仅有 `w4dj-desktop` 与 FFmpeg，工作区验收软链接和 `.pnpm-store` 也未打包。

## 2026-08-30 网易云索引状态与全局操作区修复

网易云缓存进入 ready 后现在会清理过期的“索引未就绪”状态，并以 ready 结果为权威；新一轮扫描重置旧 discoveryId，转换完成后会主动重新读取缓存状态，因此转换后无需 reload 即可显示索引已就绪。扫描超时提醒已放在任务 1 进度条右侧的索引状态位置。

“同时开始”按钮下方不再渲染扫描进度、扫描错误、取消或超时等独立说明文字；这些状态保留在任务卡进度和右侧索引状态中，按钮运行时仅保留必要的操作文案。相关前端状态、DOM 增量更新和回归测试已同步调整。

验证：前端全量 12 文件/212 项、`app.test.ts` 137 项、TypeScript、Vite 和 `git diff --check` 通过。pnpm 包装命令受本机 `ERR_PNPM_IGNORED_BUILDS` 安全策略限制，已用同一依赖树直接运行 Vitest。真实网易云数据库和 GUI 视觉验收仍待用户环境；版本保持 `3.2.0-beta.3`。

## 2026-08-30 转换前确认页统计重排

转换前确认页现在固定显示四个统计模块：预计新增、当前策略动作（覆盖/跳过/仅更新元数据）、输入歌曲数 / 输出歌曲数、错误文件。预计新增按策略计算，跳过的已有歌曲不计入，覆盖策略会计入将被替换的歌曲；四个统计入口均可打开逐曲明细，预计新增卡片不内嵌文件摘要，点击后明细只保留文件名和原因且不提供打开文件链接。预计新增与当前策略动作的数字强调色已交换。前端 `app.test.ts` 138 项、TypeScript、Vite 和 arm64 App 构建已通过，版本保持 `3.2.0-beta.3`。

## 2026-08-30 转换前预览真实输出与网易云链接对齐修复

预览后端新增 `output_files` 真实输出快照，仅包含当前输出目录中存在的非隐藏 MP3/WAV/AIFF；输出明细不再把计划生成的 AIFF 目标名当作现有文件。覆盖明细优先使用候选的旧输出路径，因此跨格式覆盖显示实际 MP3。网易云来源工具栏锚定来源标题行顶部，链接与状态文字统一 `9px / 1.35`。Rust 预览 32/32、前端全量 12 文件/215 项（app.test.ts 140/140）、TypeScript 与 diff-check 通过；最新版 arm64 App 于 2026-08-30 15:57:23 +0800 重建，版本保持 `3.2.0-beta.3`，未提交或推送。

## 2026-08-30 预览明细超链接命中区与图标对齐修复

预览明细中的文件名现在是可选择、可复制的普通文本，只有左侧 26×26 矩形图标按钮负责定位文件；整行和歌曲名称不再响应跳转或显示整行红色悬停。图标按钮覆盖全局图标右间距并固定网格尺寸，避免图标向左下偏移。回归测试验证文件名不在按钮内、点击文件名不调用定位、点击图标仍能定位；`app.test.ts` 140/140，前端全量 215/215，TypeScript 与 Vite 已通过。版本保持 `3.2.0-beta.3`，未提交或推送。

## 2026-08-30 预览链接高亮完整性修复

图标按钮的悬停/焦点高亮现在绘制在图标矩形内部（边框颜色加内阴影），不再依赖向外扩展的父级 outline，因此不会在列表滚动边界出现缺口。26×26 点击尺寸、文件名可复制和仅图标可定位的交互保持不变；前端 215/215、TypeScript、Vite 已通过，版本保持 `3.2.0-beta.3`。

## 2026-08-30 转换确认弹窗扁平布局修复

第一次将预览卡片改成内部滚动后造成了嵌套滚轮和文字压缩，现已撤销。预览卡片、逐曲列表和双栏明细全部按内容自然展开，只在整个确认弹窗超出小屏幕时保留一个外层兜底滚动区；默认桌面窗口宽度适度扩大到 `1280`，高度调整为 `800`，使主界面底部保留接近顶部红框所示的呼吸留白。布局回归、前端全量 217/217、TypeScript、Vite 与 Tauri 74/74 均通过，版本保持 `3.2.0-beta.3`。

歌曲明细框保持正常行高，默认完整展示至少 6 首；只有第 7 首及之后的内容需要在歌曲列表内滚动，因此默认窗口无需继续放大。

逐曲状态说明不再放在歌名下方，而是统一位于同一行右侧并靠右对齐。“将覆盖现有输出文件”“将跳过”“将更新元数据”和新增文件说明共用同一布局，长歌名会在中间弹性区域省略，不会再与状态文字重叠。

## 2026-08-30 转换前确认页输入/输出明细修复

“预计新增”现在可点击打开静态明细；统计卡片本身不再内嵌文件摘要，展开明细显示预计输出文件名和原因，但不提供文件打开按钮。输入/输出统计的明细弹窗改为两列，输入列显示源目录文件名并保留跳过、错误等原因，输出列显示目标目录文件名。输入、输出、操作和错误明细的整行（包括歌曲文件名）都使用系统文件管理器的“在文件夹中定位”动作：目标已存在时选中该文件，尚未生成时打开目标目录，不直接打开或播放歌曲。预计新增与当前策略动作的数字强调色已交换。该修复避免了原先把目标文件路径交给“打开输出目录”命令而导致的所有输出明细按钮失败。前端 `app.test.ts`、TypeScript、Vite 和 `git diff --check` 已通过；版本保持 `3.2.0-beta.3`。最新 arm64 App 于 2026-08-30 14:45:39 +0800 重建。
